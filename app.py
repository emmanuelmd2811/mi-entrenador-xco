...
# --- 1. CONFIGURACIÓN Y ESTILO (Sin cambios visuales) ---
...
def obtener_datos_sesion():
    """Extrae datos de session_state evitando objetos no serializables."""
    claves = ['deporte', 'modalidad', 'nombre_carrera', 'fecha_carrera', 'nivel', 'dias_w', 'configurado', 'historial_entrenamientos']
...
if "GOOGLE_API_KEY" in st.secrets:
    ...
    model = genai.GenerativeModel('models/gemini-1.5-flash') # Versión estable
else:
...
if not st.session_state['configurado']:
    ...
    with st.container(border=True):
        ...
        if st.button("🚀 Crear mi Plan Maestro"):
            ...
            guardar_en_disco(obtener_datos_sesion())
            st.rerun()
    st.stop()
    ...
    with st.form("ajuste"):
        ...
        if st.form_submit_button("Generar Plan Adaptado"):
            with st.spinner("Consultando al Coach..."):
                try:
                    prompt = f"Coach de {st.session_state['deporte']} {st.session_state['modalidad']}. Carrera {st.session_state['nombre_carrera']}. Hoy: {dia_actual_nombre}. Feedback: {resumen}. Fatiga: {fatiga}. Genera plan hasta domingo. Formato: [DIA] con secciones **ENTRENAMIENTO PRINCIPAL**, **FUERZA/MOVILIDAD**, **NUTRICIÓN**."
                    res = model.generate_content(prompt)
                    st.session_state['historial_entrenamientos'][semana_id] = res.text
                    guardar_en_disco(obtener_datos_sesion())
                    st.rerun()
                except Exception as e:
                    st.error(f"Error de la IA: {e}. Intenta de nuevo.")
    st.stop()
...
# --- 8. VISUALIZACIÓN (Algoritmo de búsqueda optimizado) ---
...
def extraer_dia_pro(dia_buscado, texto_completo):
    texto_norm = normalizar(texto_completo)
    dia_norm = normalizar(dia_buscado)
    
    # Intenta encontrar el bloque del día con diferentes marcadores
    delimitadores = [f"[{dia_norm}]", f"**{dia_norm}**", f"### {dia_norm}", f"{dia_norm}:"]
    inicio = -1
    for d in delimitadores:
        inicio = texto_norm.find(d)
        if inicio != -1:
            inicio += len(d)
            break
            
    if inicio == -1: return "Día no detallado o descanso activo."
    
    # El final es el inicio del siguiente día de la lista completa
    fin = len(texto_norm)
    for d in dias_semana_es:
        d_n = normalizar(d)
        for sep in [f"[{d_n}]", f"**{d_n}**", f"### {d_n}"]:
            pos = texto_norm.find(sep, inicio)
...
for i, nombre_dia in enumerate(dias_visibles):
    with tabs[i]:
        contenido = extraer_dia_pro(nombre_dia, plan_actual)
        if nombre_dia == dia_actual_nombre: st.success(f"⚡ OBJETIVO DE HOY")
        
        # Regex mejorada para capturar secciones incluso sin negritas perfectas
